...
(`data:image/webp` URIs in `@openreceive/provider-data`'s main bundle —
≤ 72 px WebP, 35 KB for all 37) and the 20 pay tutorials (800 px WebP, 201 KB
for all 20, in a separate chunk `loadPayTutorialImages()` imports on first
open, so a payer who never opens one never downloads them). There is no image
file to copy or serve and no asset option to set, under any bundler or with
none. The one host-facing consequence is a Content-Security-Policy with a
strict `img-src`, which must allow `data:` (the payment icons already needed
that).

Every layer of the old asset seam is gone — packaged `import.meta.url` URLs,
the `file://` console warning, `resolveAssetUrl`, `assetBaseUrl` — because
each was a workaround for bundlers that cannot resolve a file from an import,
and each handed the host one more step to get wrong: setting `asset-base-url`
also switched the compiled-in payment icons to files that were never copied,
and the checkout came up with every image broken.

Removed:

- `@openreceive/provider-data`: `payTutorialUrls`, `providerTutorialUrl`,
  `AssetUrlResolver`, `createAssetBaseUrlResolver`, `lazyAssetUrlTable`,
  `warnOnFileAssetUrl`, the `./assets/provider-icons/*` and
  `./assets/pay_tutorials/*` exports and the `dist/assets` directory.
  Registry `icon_path` values now end in `.webp`. Added
  `loadPayTutorialImages()` and `payTutorialImage(path)`; `providerIconUrls` /
  `providerIconUrl` keep their names and now answer data URIs.
- `@openreceive/browser` (and `/headless`): `AssetUrlResolver`,
  `createAssetBaseUrlResolver`, `paymentIconPaths`, `getRouteIconPath`, the
  `iconPath` field on `WizardProviderDisplay` / `WizardRouteAssetDisplay`,
  every `resolveAssetUrl` parameter and option, `assetBaseUrl` in
  `CheckoutOptions` and the element attribute options,
  `OPENRECEIVE_CHECKOUT_ELEMENT_ATTRIBUTES.assetBaseUrl`, the `./assets/*`
  export and `dist/assets`. `getPaymentMethodIcon`, `getNetworkIcon`,
  `getSwapOptionIcon`, `getRouteIcon`, `getCheckoutProviderIcon`,
  `getCheckoutProviderTutorials`, `createWizardRouteAssetDisplays` and
  `createWizardRouteDisplays` lose their resolver argument.
  `WizardProviderTutorialDisplay.image` is `string | undefined` until the
  tutorial chunk has loaded. `/headless` re-exports `loadPayTutorialImages`
  and `payTutorialImage`.
- `@openreceive/elements`: the `asset-base-url` attribute and
  `defineElements({ resolveAssetUrl })`; `renderPaymentIconHtml` always draws
  inline SVG. The tutorial dialog renders the caption alone until the images
  resolve — never an `<img>` with an empty `src`.
- `@openreceive/react`: the `assetBaseUrl` and `resolveAssetUrl` props on
  `<Checkout>`, `PaymentWizard` and the tutorial / method-grid components.
- `@openreceive/vue`, `@openreceive/svelte`, `@openreceive/angular`: the
  `assetBaseUrl` prop / input.
- The standalone build: its `assets/` directory. The tarball is
  `openreceive-checkout.js` (tutorials inlined — one file stays one file), its
  source map, `openreceive-checkout.css` and `MANIFEST.json`.
- The demos: `copy-openreceive-provider-assets-plugin.ts` and the Rails
  demo's `copy-webpack-plugin` / `import.meta.url` loader steps.

The provider images are generated by
`tools/package/generate-provider-images.mjs` (`npm run
generate:provider-images`; `check:generated` fails when stale) from
pre-compressed WebP checked in under `packages/js/provider-data/src/assets/`.
It accepts only `.webp` and enforces byte budgets — one logo ≤ 4,096 bytes and
all logos ≤ 48 KB; one tutorial ≤ 48 KB and all tutorials ≤ 240 KB — printing
the offender and the `cwebp` command that fixes it, so the bundle cannot
...
- **A standalone checkout build for hosts without a bundler.**
  ...
  load, its stylesheet and a `MANIFEST.json` of hashes. `npm run build:packages` writes it and
...
- **A Buy a Button host on plain PHP** (`examples/buttons/server/php-plain`,
  `npm run demo php`, port 3008): the no-framework vanilla shop served as static
  files plus one front controller — `public/index.php` — over `Engine`,
  `SqlPaymentRepository` on the shop's own SQLite `PDO` and an anonymous `Host`
  with the three hooks inline. The payment step is the **standalone checkout
  build** (`packages/js/elements/dist/standalone/`, the release tarball's
  contents) copied into `public/openreceive/` and served by PHP; the shared
  client's `@openreceive/elements` import is pointed at that file, so the demo
  proves the bundler-less artefact. Testkit mode uses the engine's own
  `Testing\FakeWallet` / `FakeSwapProvider` and the four `/__testkit` routes,
  with the fakes' state persisted between requests (a PHP request is a
  process). `OPENRECEIVE_E2E_STACK=php-plain` runs the shared Playwright smoke;
  `bin/ci` drives order → invoice → settle → download over HTTP; the Docker
  image runs `php -S` and says so. `check:demo-containers` learned the `php`
  kind (engine from the monorepo path repository, never Packagist; no baked
  `.env`; `ext-gmp` installed).
- **PHP documentation.** `docs/guides/quickstart-php.md` (Composer install, the
  `PaymentsSchema` migration, the three-method `Host` + `Engine` +
  `Psr15Handler` front controller, the standalone checkout tarball, why
  `<meta name="csrf-token">` is optional and what the engine's cross-site check
  does not cover, `Doctor::report()`), the `php` agent-directions payload
  (`docs/agents/php.md`, on openreceive.org at `/agent-directions/php.md`),
  a `## PHP` section in the API reference, PHP sections in host testing and
  storage, and the plain-PHP deployment row.
- **Laravel adapter (`packages/php/laravel`, Composer `openreceive/laravel`), the
  Buy a Button Laravel demo, `quickstart-laravel`.** The thin binding of the PHP
  engine for Laravel 11/12: a service provider that resolves the `Host` by
  CLASS NAME from `config/openreceive.php` (so `config:cache` works), builds
  the repository over the app's own `DB` connection and the `Service` from
  `NWC_URI` (a bound `ReceiveNwcClient` / `PriceProvider` /
  `openreceive.swap_providers` wins — the seams tests and testkit mode use),
  mounts one catch-all route under the configured prefix and middleware group
  ...
  (NWC-02 listener + periodic pass, resubscribe backoff) are the artisan twins
  of the Rails tasks. Boot warns while a placeholder is wired; a production web
  boot preflights the wallet eagerly (the secretless build commands skip it);
  the wallet's info event is remembered in the app cache
  (`wallet_info_cache_seconds`, `NostrPhpNwcReceiveClient::primeInfo()`) so a
  PHP request costs one relay round trip, not two. Orchestra Testbench suite:
  routes (419 without the CSRF header, 200 with it), authorize's Illuminate
  request, the migration on sqlite/pgsql/mysql, the config round trip, doctor,
  placeholder warnings, the fail-closed preflight. The demo
  (`examples/buttons/server/laravel`, `npm run demo laravel`, port 3005) is the
  Rails shop in Laravel 12: Eloquent models, the three hooks in
  `app/OpenReceive/Host.php`, the shared React client with `<Checkout>` from
  `@openreceive/react`, Laravel's own Vite as the dev front door over
  `php artisan serve`, Apache + Postgres in compose with a notifications
  service, testkit mode with the fakes' state persisted between PHP requests,
...
- **Composer release tooling.** `tools/release/composer-release.mjs
  ...
  split repositories `OpenReceive/openreceive-php` and `OpenReceive/laravel`
  with the `v<version>` tag, and a poll of packagist.org until the version
  lists. `release:prepare` writes `OpenReceive\Version::VERSION` and the
  Laravel constraint; `check:release` compares both. `publish-composer.yml`
  runs the push on a `v*` tag behind the `packagist` environment's approval.
- **PHP CI.** A `php-engine` job (PHP 8.2 and 8.4, Postgres and MySQL service
  containers behind `OPENRECEIVE_TEST_*_DSN`, PHPStan) per push, the plain-PHP
  demo's `bin/ci` plus browser smoke weekly, `test:php` in `test:ci:release`.
