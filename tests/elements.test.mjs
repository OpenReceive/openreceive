...
test("elements render display-safe checkout HTML", () => {
  ...
  // The BOLT11 never appears: without a host-configured decoder there is no
  // Decode link, and the invoice is not visible text either.
...
test("elements render payment wizard route choices and providers from browser state", () => {
  ...
  assert.match(startingSol, /aria-busy="true"/);
  assert.match(startingSol, /part="spinner"/);
  ...
  assert.match(startingUsdtContinue, /part="method-confirm"/);
  assert.match(startingUsdtContinue, /Preparing payment/);
  assert.match(startingUsdtContinue, /part="spinner"/);
  assert.match(
    startingUsdtContinue,
...
test("elements package exposes shared browser-owned checkout styles", () => {
  const manifest = JSON.parse(
    readFileSync(path.join(process.cwd(), "packages/js/elements/package.json"), "utf8"),
  );
  const styles = readFileSync(
    path.join(process.cwd(), "packages/js/elements/src/styles.css"),
    "utf8",
  );
  // The implementation is split by concern (M41); assert over every module.
  const source = readdirSync(path.join(process.cwd(), "packages/js/elements/src"))
    .filter((entry) => entry.endsWith(".ts"))
    .map((entry) =>
      readFileSync(path.join(process.cwd(), "packages/js/elements/src", entry), "utf8"),
    )
    .join("\n");
...
test("the invoice decode link is off until the host names a decoder", () => {
  ...
  assert.match(withDecoder, />Decode</);
  assert.match(withDecoder, /https:\/\/decoder\.example\/decode\?invoice=lnbc-decode-test/);
  ...
  assert.match(
    ...
    /decode\?lang=en&amp;invoice=lnbc-decode-test/,
  );
});
