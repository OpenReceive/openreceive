...
const guard = new URL(
  "../packages/dotnet/docker/port-guard.sh",
  import.meta.url,
).pathname;
...
function run(t, project, holder) {
  ...
  return spawnSync(
    ...
    [
      "-c",
      `source "${guard}"; require_btcpay_port_free "${project}"; echo proceeded`,
    ],
    {
...
test("the caller's own BTCPay container is not a conflict", (t) => {
  const result = run(
    t,
    "openreceive-btcpay",
    "openreceive-btcpay-btcpayserver-1",
  );
  assert.equal(result.status, 0, result.stderr);
});
...
test("the live demo names the testkit stop command when the regtest stack holds the port", (t) => {
  ...
  assert.match(
    result.stderr,
    /14180 is held by openreceive-btcpay-btcpayserver-1/,
  );
...
test("the regtest stack names the live stop command when the demo holds the port", (t) => {
  const result = run(
    t,
    "openreceive-btcpay",
    "openreceive-btcpay-live-btcpayserver-1",
  );
  assert.equal(result.status, 1);
