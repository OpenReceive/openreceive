...
import { checkPayment, scanSettledPayments } from "../../packages/js/core/src/index.ts";
...
test("payment scans page at 20, deduplicate hashes, and require settlement authority", async () => {
  ...
  const client = {
    ...
    async makeInvoice() { throw new Error("unused"); },
  ...
  const settled = await scanSettledPayments({ client, from: 0, until: 1000 });
  assert.equal(settled.length, 1);
  assert.equal(settled[0].paymentHash, hash(21));
  assert.deepEqual(requests.map((request) => request.limit), [20, 20]);
  assert.deepEqual(requests.map((request) => request.offset), [0, 20]);

  requests.length = 0;
  const preimageOnly = await checkPayment({ client, paymentHash: hash(1), clock: () => 999 });
...
test("state=settled without settled_at uses observed time and identifies its source", async () => {
  ...
  const client = {
    async preflight() { return {}; },
    async makeInvoice() { throw new Error("unused"); },
    async lookupInvoice() { return { payment_hash: paymentHash, state: "settled" }; },
    async listTransactions() { return { transactions: [] }; },
  };
  const result = await checkPayment({ client, paymentHash, clock: () => 777 });
