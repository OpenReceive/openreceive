...
import { TESTKIT_PREIMAGE, createTestkitReceiveClient } from "@openreceive/testkit";
...
test("testkit receive client scripts deterministic transaction sequences", async () => {
  ...
  wallet.scriptTransactionSequence({ payment_hash: invoice.payment_hash }, [
    { state: "pending" },
  ...
  const settled = firstTransaction(
    await wallet.listTransactions({
      type: "incoming",
      unpaid: true,
      from: invoice.created_at,
...
test("testkit receive client enforces make invoice validation vectors", async () => {
  for (const vector of MAKE_INVOICE_VALIDATION_VECTORS.cases) {
    ...
    if (vector.expected.valid) {
      ...
      await assert.rejects(() => wallet.makeInvoice(request), Error, vector.name);
