...
| `@openreceive/node` | Implemented | Receive-checkout adapter around `@getalby/sdk` NWC with canonical wallet-error normalization, payment notification listener helper, and package-owned Postgres invoice persistence/migration helpers. |
| `@openreceive/express` | Implemented | Mounted HTTP routes, auth hooks, signed event URLs, idempotent create/refresh, lookup, SSE events, settlement action hook, separate settlement polling runner, payment notification runner with startup logs, and static rates/routes/providers helpers. |
...
| Next.js Fullstack Hello Fruit | Implemented | App Router route handlers and a thin React client that delegates checkout UI/state to `@openreceive/react`. |
