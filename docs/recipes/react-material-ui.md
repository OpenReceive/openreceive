...
> This recipe renders a checkout **snapshot** (resume mode). The default
> integration is create mode — `<Checkout orderId={order.id} prefix="/openreceive" />` —
> which prepares, mints, and polls by itself; everything below about components
> and classNames applies to both modes.
