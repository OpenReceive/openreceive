...
- `@openreceive/node` for receive-only NWC, package-owned persistence, CLI
  setup, migrations, doctor checks, and the combined backend worker.
- `@openreceive/express` for mounted HTTP routes, auth hooks, signed event
  URLs, SSE events, backend settlement verification, and settlement actions.
