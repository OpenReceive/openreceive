...
The sections above say what each group is for. The full inventory — generated
from the entry module so a promoted or dropped symbol cannot silently diverge
from the docs — lives in
...
inventory in [docs/internal/headless-surface.md](../internal/headless-surface.md)
cannot drift from the entry module.
