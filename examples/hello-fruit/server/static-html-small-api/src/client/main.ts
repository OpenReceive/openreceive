...
let purchasedStickers: readonly HelloFruitPurchasedSticker[] = [];
...
function renderCheckout(orderId: string): void {
  ...
  checkoutElement.addEventListener(OPENRECEIVE_CHECKOUT_ELEMENT_EVENTS.settled, (event) => {
    if (currentOrder === undefined || completedOrderId === orderId) return;
    const detail = (event as CustomEvent<{ state?: CheckoutState }>).detail;
    if (detail?.state !== undefined) {
      settledCheckoutState = detail.state;
    }
...
async function revealFulfilledDelivery(orderId: string): Promise<void> {
  ...
  showStickerModal(stickers);
}

function setError(message: string): void {
  const errorEl = requireElement("error");
  const hidden = message === "";
  if (errorEl.textContent === message && errorEl.classList.contains("hidden") === hidden) {
    return;
  }
  if (message !== "") {
  ...
  errorEl.classList.toggle("hidden", hidden);
}

function showStickerModal(stickers: readonly HelloFruitPurchasedSticker[]): void {
  closeStickerModal();
  logDemo("sticker_modal.show", "Showing sticker download modal.", {
    productIds: stickers.map((sticker) => sticker.productId),
  });
  purchasedStickers = stickers;
  ...
  gallery.className = "grid grid-cols-2 sm:grid-cols-3 gap-2 justify-items-center";
  const downloads = document.createElement("div");
  downloads.className = "grid gap-2";
  for (const sticker of stickers) {
    const image = document.createElement("img");
    image.className = "w-full max-w-[180px] aspect-square";
    image.src = sticker.objectUrl;
    image.alt = "";
    gallery.append(image);

    const download = document.createElement("a");
    download.className = "btn btn-outline justify-between";
    download.href = sticker.objectUrl;
    download.download = sticker.filename;
    download.textContent = `${sticker.name} sticker${sticker.quantity > 1 ? ` ×${sticker.quantity}` : ""}`;
    downloads.append(download);
  }

  const copy = helloFruitStickerModalCopy(stickers);
  ...
  modal.append(gallery, title, detail, downloads);
  if (transactionDetails !== null) modal.append(transactionDetails);
...
function closeStickerModal(): void {
  ...
  revokeHelloFruitStickers(purchasedStickers);
  purchasedStickers = [];
}
