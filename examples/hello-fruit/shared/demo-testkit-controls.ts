...
export const HELLO_FRUIT_TESTKIT_PREFIX = "/__testkit";

export interface HelloFruitTestkitFixtures {
 ...
 */
export function mountHelloFruitTestkitControls(
  app: Express,
  fixtures: HelloFruitTestkitFixtures | undefined,
): void {
  if (fixtures === undefined) {
    app.use(HELLO_FRUIT_TESTKIT_PREFIX, (_req: Request, res: Response) => {
      res.status(404).json({ code: "NOT_FOUND", message: "Not found.", retryable: false });
    });
    return;
  }
  app.use(HELLO_FRUIT_TESTKIT_PREFIX, createTestkitControlRouter(fixtures));
}

function createTestkitControlRouter(fixtures: HelloFruitTestkitFixtures): Router {
