...
def create_app(env: dict[str, str] | None = None) -> FastAPI:
    ...
    logging.basicConfig(level=env.get("LOG_LEVEL", "INFO").upper(), format="%(name)s %(levelname)s %(message)s")
    ...
    def amount_for(reference: str) -> dict[str, str] | None:
        ...
        if order is None:
            return None
        return {"currency": order.row["currency"], "value": shop.format_amount(order.row["total_cents"]), "description": order.description()}
    ...
    def on_paid(settlement: PaymentSettlement) -> None:
        ...
        if shop.claim_order_paid(settlement.connection, settlement.reference, settlement.paid_at, settlement.payment_hash):
            log.info("openreceive.on_paid reference=%s payment_hash=%s — order paid, downloads unlocked", settlement.reference, settlement.payment_hash)
    ...
    router = openreceive_router(
        ...
        env=env,
        **(
            {"nwc_client": testkit[0], "swap_providers": [testkit[1]], "price_provider": StaticPriceProvider()}
            if testkit
            else {}
        ),
    )
    app = FastAPI(lifespan=openreceive_lifespan(host, engine=engine), docs_url=None, redoc_url=None, openapi_url=None)
    app.include_router(router, prefix=OPENRECEIVE_PREFIX)
    app.include_router(testkit_router(testkit), prefix="/__testkit")
    if testkit:
        log.info("openreceive.testkit wallet mode: in-memory fakes, no NWC connection (/__testkit live)")
    ...
    def visitor(request: Request, response: Response) -> dict[str, Any]:
        ...
        response.set_cookie(
            shop.SHOP_COOKIE, cookie, max_age=shop.COOKIE_MAX_AGE_SECONDS, path="/", httponly=True,
    ...
    def bootstrap(request: Request, response: Response) -> dict[str, Any]:
        ...
        catalog = [
            {"sku": p["sku"], "name": p["name"], "price_cents": p["price_cents"], "image_url": shop.image_url(p["image_name"])}
            for p in store.catalog()
        ]
        return {"shop": {"currency": "USD", "max_per_sku": shop.MAX_PER_SKU, "openreceive_prefix": OPENRECEIVE_PREFIX, "catalog": catalog, "visitor": {"public_ref": user["public_ref"]}}}

    @app.post("/shop/orders", status_code=201)
    async def create_order(request: Request, response: Response) -> Any:
        user = visitor(request, response)
        body = await request.json() if await request.body() else {}
        order = store.create_order(body.get("items") if isinstance(body, dict) else None, str(user["id"]))
        if order is None:
            return JSONResponse({"error": "Your cart is empty."}, status_code=422, headers=dict(response.headers))
    ...
    def show_order(request: Request, response: Response, reference: str) -> Any:
        ...
        if order is None:
            return JSONResponse({"error": "Not found."}, status_code=404, headers=dict(response.headers))
        return shop.order_payload(order)
    ...
    def download(request: Request, response: Response, reference: str, sku: str) -> Any:
        ...
        if order is None:
            return JSONResponse({"error": "Not found."}, status_code=404, headers=dict(response.headers))
        if not order.paid:
            return JSONResponse({"error": "Not paid."}, status_code=403, headers=dict(response.headers))
        ...
        if name is None or not (shop.ARTWORK_DIR / name).exists():
            return JSONResponse({"error": "Not found."}, status_code=404, headers=dict(response.headers))
        return FileResponse(shop.ARTWORK_DIR / name, media_type="image/webp", filename=name, headers=dict(response.headers))
    ...
    def recent_orders() -> JSONResponse:
        ...
        return JSONResponse(
            {"orders": [shop.feed_payload(order, buyer) for order, buyer in orders], "totals": totals},
