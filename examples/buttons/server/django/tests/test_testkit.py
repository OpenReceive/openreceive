...
def test_every_action_is_404_when_testkit_is_off(
    monkeypatch: pytest.MonkeyPatch, browser: Browser
) -> None:
...
def test_the_control_surface_drives_the_fakes(browser: Browser) -> None:
    ...
    settled = browser.post(
        "/__testkit/settle", {"payment_hash": minted["payment_hash"]}, csrf=False
    )
    assert (
        settled.status_code == 200
        and settled.json()["transaction"]["transaction_state"] == "settled"
    )
    ...
    bad = browser.post(
        "/__testkit/swap-step", {"pay_in_asset": "USDT_TRON", "state": "nope"}, csrf=False
    )
    ...
    armed = browser.post(
        "/__testkit/swap-step",
        {"pay_in_asset": "USDT_TRON", "state": "refund_required"},
        csrf=False,
    )
