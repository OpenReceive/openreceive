...
import { DJANGO_CSRF_HEADER, installDjangoCsrfMeta } from "./csrf.ts";

/**
 * The Django host: the packaged React `<Checkout>` over the Django adapter.
 *
 ...
 * Django-mounted OpenReceive app. The catalog, the cart, the receipt and
...
export const ShopApp: React.FC = () => {
  ...
  useEffect(() => {
    ...
    loadShopBootstrap()
      .then((bootstrap) => {
        if (cancelled) return;
        // The bootstrap response set Django's csrftoken cookie; every POST from
        // here on — the shop's own and the checkout's — carries it as X-CSRFToken.
        installDjangoCsrfMeta();
    ...
    <MantineProvider theme={shopTheme} defaultColorScheme="light">
      <main className="or-page">
        <div className="or-page-inner">
          ...
          <Text className="or-page-note">Django + Postgres. React.</Text>
        </div>
      </main>
    </MantineProvider>
  );
      ...
      <>
        ...
        <div className="or-shop-stage or-checkout">
          ...
          <Stack className="or-checkout-pay" gap="sm">
            <Checkout
              // Django's CSRF middleware reads X-CSRFToken (Rails and Laravel
              // read the default X-CSRF-Token); the token itself comes from
              // <meta name="csrf-token">, which src/client/csrf.ts filled in.
              csrfHeader={DJANGO_CSRF_HEADER}
              defaultTheme="light"
