...
const TESTKIT_WALLET_PUBKEY = "f".repeat(64);
const TESTKIT_RELAY = "wss://relay.test.openreceive.local";
...
export interface TestkitReceiveClientOptions {
  /**
   * Clock in unix seconds. Defaults to the real clock, matching what
   * reconciliation uses: a fixed low value would place every minted invoice
   * past expiry plus grace, so a reconcile pass would close attempts the test
   * still considers pending. Pass one for deterministic timestamps.
   */
  readonly now?: () => number;
...
export class TestkitReceiveClient implements OpenReceiveReceiveNwcClient {
  ...
  constructor(options: TestkitReceiveClientOptions = {}) {
    ...
    for (const invoice of options.initialInvoices ?? []) {
      this.#store({
        ...
        expires_at:
          invoice.expires_at ?? (invoice.created_at ?? this.#now()) + this.#defaultExpirySeconds,
  ...
  #nextScriptedTransaction(stored: TestkitStoredInvoice): NwcTransaction | undefined {
