...
function SwapRefundForm(props: {
  ...
  const formatError = getSwapRefundAddressError(props.payInAsset, address, props.networkLabel);
  const addressError = address.length === 0 ? "Enter a refund address." : formatError;
  const confirm =
    address.length > 0 &&
