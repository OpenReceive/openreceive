...
export function renderDrizzleFiles(options: ScaffoldPaymentsOptions): ScaffoldFile[] {
  ...
  const constraints = [
    ...
    `    check("${indexName("status_check")}", sql\`${openReceivePaymentsStatusCheckSql()}\`),`,
    `    check("${indexName("hash_check")}", sql\`${openReceivePaymentsHashCheckSql(options.dialect)}\`),`,
  ].join("\n");
...
export const openReceiveMeta = sqliteTable("${options.metaTableName}", {
...
// The durable reconcile gate every worker on this database shares
// (key/value/rev compare-and-set). Same host database, never a second one.
export const openReceiveMeta = pgTable("${options.metaTableName}", {
  key: text("key").primaryKey(),
  value: text("value").notNull(),
