...
function constraintsSql(options: ScaffoldPaymentsOptions): string {
  const statusCheck = openReceivePaymentsStatusCheckSql();
  const hashCheck = openReceivePaymentsHashCheckSql(options.dialect);
  const seed = openReceivePaymentsSeedSql(options.dialect, options.metaTableName);
...
ALTER TABLE "${options.tableName}" ADD CONSTRAINT "${openReceivePaymentsIndexName(options.tableName, "status_check")}" CHECK (${statusCheck});
ALTER TABLE "${options.tableName}" ADD CONSTRAINT "${openReceivePaymentsIndexName(options.tableName, "hash_check")}" CHECK (${hashCheck});
${seed};
`;
  }
  return `-- Generated by \`npx openreceive scaffold payments\`.
