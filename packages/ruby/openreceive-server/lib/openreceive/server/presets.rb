...
module OpenReceive
  module Server
    ...
    module Presets
      ...
      # Policy for an guest-checkout site (no accounts) / paywall: anyone may create a checkout, per-order reads
      # and swap actions are gated on the order's capability token (the httpOnly cookie set on create
      # is enough), and admin sweep is denied unless `allow_sweep` opts in. Byte-identical to the JS
      # `guestCheckout`: checkout.create → true; invoice.sweep → allow_sweep&.call(ctx) or false; else
      # → ctx[:token_valid].
      #
      ...
      # always create a checkout, may sweep only when `is_admin` allows, and may read/act on an order
      # per `owns_order` (falling back to the order token via ctx[:token_valid] when `owns_order` is not
      # supplied). Byte-identical to the JS `withUser`.
      #
